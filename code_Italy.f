c	***************************************************************
c	HDC model of Covid-19 for Italy
...
c	idum=0.0
        ...
        open(14,file='covid_Italy_input.in',status='old')	
...
c      Used parameter of our simulation
        ...
        read(14,*)it16d         ! Day-16
	read(14,*)it86d         ! Day-86
	read(14,*)it230d        ! Day-230
	read(14,*)it278d        ! Day-278
	read(14,*)it325d        ! Day-325
	read(14,*)it402d        ! Day-402
	read(14,*)it435d        ! Day-435
...
c      Implementation of lock-down
        if(k.lt.it16d)then !16days (1-16)day Unrestricted
            amvsc(i,j)=amv_sc
            amvsy(i,j)=amv_sy
            amvso(i,j)=amv_so
            amvec(i,j)=amv_ec
            amvey(i,j)=amv_ey
            amveo(i,j)=amv_eo  
         else if(k.ge.it16d.and.k.lt.it86d)then !70days (17-86)day Lock-1
         ...
         else if(k.ge.it86d.and.k.lt.it230d)then !144 days (87-230)day Lock-2
         ...
         else if(k.ge.it230d.and.k.lt.it278d)then !48 days (231-278)day Unrestricted
            amvsc(i,j)=amv_sc
            amvsy(i,j)=amv_sy
            amvso(i,j)=amv_so
            amvec(i,j)=amv_ec
            amvey(i,j)=amv_ey
            amveo(i,j)=amv_eo  
         else if(k.ge.it278d.and.k.lt.it325d)then !47days (289-325)day Lock-2
            amvsc(i,j)=flckdn2*amv_sc
            amvsy(i,j)=flckdn2*amv_sy
            amvso(i,j)=flckdn2*amv_so
            amvec(i,j)=flckdn2*amv_ec
            amvey(i,j)=flckdn2*amv_ey
            amveo(i,j)=flckdn2*amv_eo 
         else if(k.ge.it325d.and.k.lt.it402d)then !77days (326-402)day Lock-3
         ...
         else if(k.ge.it402d.and.k.lt.it435d)then !157days (403-435)day lock Lock-1
            amvsc(i,j)=flckdn1*amv_sc
            amvsy(i,j)=flckdn1*amv_sy
            amvso(i,j)=flckdn1*amv_so
            amvec(i,j)=flckdn1*amv_ec
            amvey(i,j)=flckdn1*amv_ey
            amveo(i,j)=flckdn1*amv_eo 
          else ! 190 days(436-625)days Lock-2
            amvsc(i,j)=flckdn2*amv_sc
            amvsy(i,j)=flckdn2*amv_sy
            amvso(i,j)=flckdn2*amv_so
            amvec(i,j)=flckdn2*amv_ec
