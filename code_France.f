c	***************************************************************
c	HDC model of Covid-19 for France
...
c	idum=0.0
        ...
        open(14,file='covid_France_input.in',status='old')	
...
c      Used parameter of our simulation
        ...
        read(14,*)it20d         ! Day-20
	read(14,*)it75d         ! Day-75       
	read(14,*)it200d        ! Day-200
	read(14,*)it249d        ! Day-249
	read(14,*)it295d        ! Day-295
	read(14,*)it403d        ! Day-403
	read(14,*)it460d        ! Day-460
...
c      Implementation of lock-down
        if(k.lt.it20d)then !20days (1-20)day Unrestricted
            amvsc(i,j)=amv_sc
            amvsy(i,j)=amv_sy
            amvso(i,j)=amv_so
            amvec(i,j)=amv_ec
            amvey(i,j)=amv_ey
            amveo(i,j)=amv_eo  
         else if(k.ge.it20d.and.k.lt.it75d)then !55 days (21-75)day Lockdown-1
         ...
         else if(k.ge.it75d.and.k.lt.it200d)then !125 days (76-200)day Lockdown-2
         ...
         else if(k.ge.it200d.and.k.lt.it249d)then !49 days (201-249)day Unrestricted
            amvsc(i,j)=amv_sc
            amvsy(i,j)=amv_sy
            amvso(i,j)=amv_so
            amvec(i,j)=amv_ec
            amvey(i,j)=amv_ey
            amveo(i,j)=amv_eo  
         else if(k.ge.it249d.and.k.lt.it295d)then !47days (250-295)day Lockdown1
            amvsc(i,j)=flckdn1*amv_sc
            amvsy(i,j)=flckdn1*amv_sy
            amvso(i,j)=flckdn1*amv_so
            amvec(i,j)=flckdn1*amv_ec
            amvey(i,j)=flckdn1*amv_ey
            amveo(i,j)=flckdn1*amv_eo 
         else if(k.ge.it295d.and.k.lt.it403d)then !108 days (296-403)day Lockdown3
         ...
         else if(k.ge.it403d.and.k.lt.it460d)then !57 days (404-460)day Lockdown-1
            amvsc(i,j)=flckdn1*amv_sc
            amvsy(i,j)=flckdn1*amv_sy
            amvso(i,j)=flckdn1*amv_so
            amvec(i,j)=flckdn1*amv_ec
            amvey(i,j)=flckdn1*amv_ey
            amveo(i,j)=flckdn1*amv_eo 
          else ! 165days(461-625)days Lockdown-3
            amvsc(i,j)=flckdn3*amv_sc
            amvsy(i,j)=flckdn3*amv_sy
            amvso(i,j)=flckdn3*amv_so
            amvec(i,j)=flckdn3*amv_ec
